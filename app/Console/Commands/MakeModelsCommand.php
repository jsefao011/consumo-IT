...
class MakeModelsCommand extends Command
{
    ...
    protected function generateTable($table)
    {
        ...
        class Acceso extends Model
        {
            ...
            {
                return $this->hasMany('App\Entities\local\Acceso','Cod_Padre');
            }
        }*/
        ...
        if($archivo = fopen($nombre_archivo, "a")) {
            ...
            fwrite($archivo, "{\n");
            fwrite($archivo, "    const TABLE = '".$table."';\n");
            fwrite($archivo, "    public static \$ALL_COLUMN = ['".$columns."'];\n");
            foreach ($columnsArray as $columna) {
                fwrite($archivo, "    const ". $columna . " = '". $columna . "';\n");
            }
            fwrite($archivo, "\n");
            fwrite($archivo, "    public static function withTable(\$column){\n");
            fwrite($archivo, "          return self::TABLE.'.'.\$column;\n");
            fwrite($archivo, "    }\n");
            fwrite($archivo, "\n");
            fwrite($archivo, "    public static function aliseTable(\$alise){\n");
            fwrite($archivo, "          return self::TABLE.' as '.\$alise;\n");
            fwrite($archivo, "    }\n");
            fwrite($archivo, "\n");
            fwrite($archivo, "    public static function withAlise(\$alise, \$column){\n");
            fwrite($archivo, "          return \$alise.'.'.\$column;\n");
            fwrite($archivo, "    }\n");
            fwrite($archivo, "    public static function allColumnTable(){\n");
            fwrite($archivo, "          foreach (self::\$ALL_COLUMN as &\$colum){\n");
            fwrite($archivo, "              \$colum = self::TABLE . '.' . \$colum;\n");
            fwrite($archivo, "          }\n");
            fwrite($archivo, "          return self::\$ALL_COLUMN;\n");
            fwrite($archivo, "    }\n");
            fwrite($archivo, "\n");
            fwrite($archivo, "    public static function allColumnTableWith(\$allColumn){\n");
            fwrite($archivo, "          foreach (self::\$ALL_COLUMN as &\$colum){\n");
            fwrite($archivo, "              \$colum = self::TABLE . '.' . \$colum;\n");
            fwrite($archivo, "          }\n");
            fwrite($archivo, "          return array_merge(self::\$ALL_COLUMN, \$allColumn);\n");
            fwrite($archivo, "    }\n");
            fwrite($archivo, "}\n");
            fclose($archivo);
        }
        ...
        $nombre_archivo = app_path()."/Entities/local2/".strtoupper($table).".php";
